...
function makeMagicCircle() {
  ...
  for (i = 0; i < dice; i++) appendRandomElement(circle, maxChildsPerElements);

  /* generates subcircles */
  dice =
    minNumberOfSubcircles +
    Math.floor(Math.random() * (maxNumberOfSubcircles - minNumberOfSubcircles));
...
function appendCircle(element, maxSubcircles) {
  ...
  if (Math.random() > chanceForFormulas) {
    ...
    // formula.innerHTML = getRandomLongFormula();
...
function moreCircles() {
  for (var i = 0; i < 10; i++) makeMagicCircle();
